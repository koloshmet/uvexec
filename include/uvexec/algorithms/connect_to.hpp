...
#include "connect_to_op_state.hpp"
...
public:
    ...
    TConnectToSender(TInSender inSender, TFn fun)
        : InSender(std::move(inSender)), Fun(std::move(fun))
    {}

    template <stdexec::receiver TRec>
    friend auto tag_invoke(stdexec::connect_t, TConnectToSender s, TRec&& rec)
            noexcept(std::is_nothrow_move_constructible_v<TFn>) {
        return TConnectToOpState<TInSender, TFn, std::decay_t<TRec>>{
                std::move(s.InSender), std::move(s.Fun), std::forward<TRec>(rec)};
    }

    friend auto tag_invoke(stdexec::get_env_t, const TConnectToSender& s) noexcept {
        return stdexec::get_env(s.InSender);
    }

    template <typename TEnv>
    friend auto tag_invoke(stdexec::get_completion_signatures_t, const TConnectToSender&, const TEnv&) noexcept {
...
struct connect_to_t {
    ...
    auto operator()(TEp&& ep, TFn fn) const noexcept(
            std::is_nothrow_invocable_v<connect_to_t, NUvExec::TJustSender<std::decay_t<TEp>>, TFn>)
            -> stdexec::sender auto {
    ...
    auto operator()(TFn fun) const noexcept(std::is_nothrow_move_constructible_v<TFn>) {
        return NUvExec::TArgBinder<TFn, connect_to_t>(std::move(fun));
    }
    ...
    auto operator()(TSender&& sender, TFn fn) const noexcept(std::is_nothrow_move_constructible_v<TFn>)
        ...
        return stdexec::transform_sender(
                d, NUvExec::TConnectToSender<TSender, TFn>{std::forward<TSender>(sender), std::move(fn)});
    }
};

inline constexpr connect_to_t connect_to;
